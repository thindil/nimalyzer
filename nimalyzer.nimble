...
task debug, "builds the project in debug mode":
  ...
  exec "nim c -d:debug --styleCheck:hint --spellSuggest:auto --errorMax:0 --warning:BareExcept:off --outdir:" &
      binDir & " " & srcDir & DirSep & "nimalyzer.nim"

task release, "builds the project in release mode":
  exec "nimble install -d -y"
  exec "nim c -d:release --passc:-flto --passl:-s --warning:BareExcept:off --outdir:" & binDir & " " &
...
task releasewindows, "builds the project in release mode for Windows 64-bit":
  exec "nimble install -d -y"
  exec "nim c -d:mingw --os:windows --cpu:amd64 --amd64.windows.gcc.exe:x86_64-w64-mingw32-gcc --amd64.windows.gcc.linkerexe=x86_64-w64-mingw32-gcc  -d:release --passc:-flto --passl:-s --warning:BareExcept:off --outdir:" &
      binDir & " " & srcDir & DirSep & "nimalyzer.nim"

task tools, "builds the project's tools":
  if not fileExists(bindir & DirSep & "nimalyzer" & ExeExt):
    exec "nimble install -d -y"
  exec "nim c -d:release --passc:-flto --passl:-s --styleCheck:hint --spellSuggest:auto --errorMax:0 --warning:BareExcept:off --outdir:" &
